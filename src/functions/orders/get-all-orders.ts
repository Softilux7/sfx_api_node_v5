...
import { getLinkedPartsFn } from './get-linked-parts'

export async function getAllOrdersFn(
  idTecnico: string,
  idBase: number,
  status: string,
  options?: {
  ...
  SELECT DISTINCT
    ...
    c.OBSDEFEITOATS, c.OBSDEFEITOCLI, c.DEPARTAMENTO, c.STATUS, c.CDSTATUS,
...
`
  ...
  const formattedData = await Promise.all(
    chamados.map(async chamado => {
      const partsData = await getLinkedPartsFn(chamado.ID_BASE, chamado.SEQOS)
      return {
        sequence: chamado.id,
        seqos: chamado.SEQOS,
        type: chamado.NMOSTP,
